...
const drawNet = (canvas: HTMLCanvasElement, netWidth: number, netColor: string) => {

    const drawRect = (context: CanvasRenderingContext2D, x: number, y: number, width: number, height:number, color: string) => {
...
const drawCircle = (context: CanvasRenderingContext2D, x: number, y: number, color: string, ballRadius: number) => {
    context.fillStyle = color;
    context.beginPath();
...
// Desenhando o texto 
const drawText = (text: string, x: number, y: number, color: string, fontSize: number = 60, fontWeight: string = 'bold', font: string = 'Roboto') => {
    context.fillStyle = color;
...
// Desenhando o paddle
const createPaddle = (context: CanvasRenderingContext2D, x: number, y: number, width: number, height:number, color: string) => {
    return {x, y, width, height, color, score: 0}
}

// Criando objeto "bola"

const createBall = (x: number, y: number, radius: number, velocityX: number, velocityY: number, color: string) => {
...
const collision = (b: { x: number, y: number, radius: number }, p: { x: number, y: number, width: number, height: number }): boolean => {
